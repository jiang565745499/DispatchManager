...
namespace DispatchManager.Components.Pages
{
    public partial class Tasks
    {
        ...
        private List<DispatchTask> SelectedRows { get; set; } = [];
    ...
    "健康检查"
        ...
        private Task<QueryData<DispatchTask>> OnQueryAsync(QueryPageOptions options)
        {
            try
            {
                var tasks = TaskServicesManager.ToList().ToTasksModelList();
                List<DispatchTask> ts = MainDb.GetAllDispatchTask();
                tasks.ForEach(task =>
                {
                    ...
                    if (ts != null)
                    {
                        ...
                        task.FNo = ts.FNo;
                        task.IsLog = ts.IsLog;
                        task.FISRetry = ts.FISRetry;
                        task.DllPath = ts.DllPath;
                        task.MethodName = ts.MethodName;
                        task.IsDllTask = ts.IsDllTask;
                    }
                });
                ...
                if (!string.IsNullOrEmpty(options.SearchText))
                {
                    ...
                    tasks = tasks.Where(task =>
                        (task.FNo.ToString()?.Contains(searchText) ?? false) ||
                        (task.Name?.ToLower().Contains(searchText) ?? false) ||
                        (task.Trigger?.ToLower().Contains(searchText) ?? false) ||
                        (task.ApiUrl?.ToLower().Contains(searchText) ?? false) ||
                        (FormatClassName(task.ClassID)?.ToLower().Contains(searchText) ?? false) ||
                        (FormatResult(task.LastRunResult)?.ToLower().Contains(searchText) ?? false) ||
                        (FormatStatus(task.Status)?.ToLower().Contains(searchText) ?? false)
                    ).ToList();
                }
        ...
        private Task<bool> OnSaveAsync(DispatchTask model, ItemChangedType changedType)
        {
            try
            {
                ...
                if (changedType == ItemChangedType.Add)
                {
                    if (MainDb.GetDispatchTaskByName(model.Name) != null
                        || MainDb.GetDispatchTaskKingDeeByName(model.Name) != null)
                    {
                        toastService.Show(new ToastOption()
                        {
                            PreventDuplicates = true,
                ...
                DispatchTaskView dispatchTaskView = new DispatchTaskView(model);
                var TaskClass = MainDb2.GetDispatchClassByID(model.ClassID);
                dispatchTaskView.Y9Key = TaskClass!.Y9Key;
                ScheduleUtils.AddTask(dispatchTaskView);
        ...
        private Task<bool> OnDeleteAsync(IEnumerable<DispatchTask> models)
        {
            try
            {
                // 演示模式下禁止删除内置任务
        ...
        private bool OnShowButtonCallback(DispatchTask model) => !IsDemo && !Jobs.Any(i => i == model.Name);
        ...
        private Task OnPause(DispatchTask model)
        {
        ...
        private Task OnDisable(DispatchTask model)
        {
            var task = TaskServicesManager.ToList().FirstOrDefault(i => i.Name == model.Name);
            if (task != null)
            {
        ...
        private Task OnRun(DispatchTask model)
        {
            var task = TaskServicesManager.ToList().FirstOrDefault(i => i.Name == model.Name);
            if (task != null)
            {
        ...
        private async Task OnLog(DispatchTask model)
        {
        ...
        private static bool OnCheckTaskStatus(DispatchTask model) => model.Status != SchedulerStatus.Disabled;

        private static bool OnCheckTaskStatus2(DispatchTask model) => true;
        ...
        /// 获取状态徽章样式类
        /// </summary>
        private static string GetStatusBadgeClass(SchedulerStatus status) => status switch
        {
            SchedulerStatus.Running => "badge-success",
            SchedulerStatus.Ready => "badge-danger",
            SchedulerStatus.Disabled => "badge-danger",
            _ => "badge-info"
        };
