...
namespace DispatchManager.Core.Libs
{
    public class SyncHSBGBL
    {
        ...
        /// <returns></returns>
        public async Task DisplayBL1(DispatchTaskView taskView)
        {
            try
            {
                var task = taskView.dispatchTask;
                string y9Url = $"http://192.168.1.70:42068/PCodeClient/api.ashx?cmd=api568140";
            ...
            {
                LogHelperUtil.WriteInterError(new Logging.Custom.LogContent($"同步 DisplayBL1 数据时出错: {ex.Message}", 0), ex);
            }
        }

        public async Task DisplayBL2(DispatchTaskView taskView)
        {
            try
            {
                var task = taskView.dispatchTask;
            ...
            {
                LogHelperUtil.WriteInterError(new Logging.Custom.LogContent($"同步 DisplayBL2 数据时出错: {ex.Message}", 0), ex);
            }
        }


        public async Task DisplayBL3(DispatchTaskView taskView)
        {
            try
            {
                var task = taskView.dispatchTask;
                ...
                using var client = new HttpClient();
                {
                    ...
                    do
                    {
                        ...
                        foreach (var item in dsArray)
                        {
                            ...
                            {
                                LogHelperUtil.WriteInterError(new Logging.Custom.LogContent($"msg.ds项类型异常，实际类型: {item.ValueKind}", task.ID));
                                continue;
                            }

                            if (!item.TryGetProperty("status", out var statusElement))
                            {
                                LogHelperUtil.WriteInterError(new Logging.Custom.LogContent("msg.ds项缺少status字段", task.ID));
                                continue;
                            }
            ...
            {
                LogHelperUtil.WriteInterError(new Logging.Custom.LogContent($"同步 DisplayBL3 数据时出错: {ex.Message}", 0), ex);
            }
        }
        ...
        public static async Task<string> DisplayHD(DispatchTask task, string postResult, int retryCount)
        {
            ...
            using var client = new HttpClient();
            {
                ...
                var ResultLast = await client.PostAsync("http://192.168.1.70:42068/PCodeClient/api.ashx?cmd=api568143", new StringContent(postResult, Encoding.UTF8, "application/json"));
