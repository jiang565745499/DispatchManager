    ...
    public class Sync_YZ_asft311
    {
        ...
        public async Task Asft31101_YZ(DispatchTaskView taskView)
        {
            ...
            try
            {
                ...
                string y9Url = $"http://192.168.190.68:43068/PCodeClient/api.ashx?cmd=api568158";
            ...
            {
                LogHelperUtil.WriteInterError(new Logging.Custom.LogContent($"同步 Asft31101_YZ 数据时出错: {ex.Message}", 0), ex);
            }
        }
        ...
        public async Task Asft31102_YZ(DispatchTaskView taskView)
        {
            ...
            {
                LogHelperUtil.WriteInterError(new Logging.Custom.LogContent($"同步 Asft31102_YZ 数据时出错: {ex.Message}", 0), ex);
            }
        }
        ...
        public async Task Asft31103_YZ(DispatchTaskView taskView)
        {
            ...
            {
                LogHelperUtil.WriteInterError(new Logging.Custom.LogContent($"同步 Asft31103_YZ 数据时出错: {ex.Message}", 0), ex);
            }
        }
        ...
        public static async Task<string> Asft311HD(DispatchTask task, string postResult, int retryCount)
        {
            ...
            using var client = new HttpClient();
            {
                ...
                var ResultLast = await client.PostAsync("http://192.168.190.68:43068/PCodeClient/api.ashx?cmd=api568159", new StringContent(postResult, Encoding.UTF8, "application/json"));
